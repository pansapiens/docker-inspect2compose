...
def transform_to_compose(service_name: str, inspect_data: Dict[str, Any], include_path_env: bool) -> OrderedDict:
    def get_ports() -> List[str]:
        ports = inspect_data['NetworkSettings']['Ports']
        return [f"{host_port['HostPort']}:{container_port.split('/')[0]}" 
                for container_port, host_ports in ports.items() if host_ports is not None 
                for host_port in host_ports]

    def get_volumes() -> List[str]:
        return [f"{mount['Source']}:{mount['Destination']}" for mount in inspect_data['Mounts']]
...
def merge_compose(existing_compose: Dict[str, Any], new_service: Dict[str, Any]) -> Dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Transform Docker container info to Docker Compose service definition.")
    parser.add_argument("container", help="ID or name of the running Docker container")
    parser.add_argument("--output", "-o", help="Output file to write the Docker Compose definition. Use '-' for stdout.", default='-')
    parser.add_argument("--include-path-env", action="store_true", help="Include the PATH environment variable in the output")
    parser.add_argument("--add-to", help="Path to an existing docker-compose.yml file to add the new service to.")
    args = parser.parse_args()

    try:
        inspect_data = get_container_info(args.container)
        service_name = inspect_data['Name'].strip('/')
        new_service = transform_to_compose(service_name, inspect_data, args.include_path_env)

        if args.add_to:
            existing_compose = load_existing_compose(args.add_to)
            updated_compose = merge_compose(existing_compose, new_service)
            write_compose(updated_compose, args.output)
        else:
            write_compose(new_service, args.output)

        logging.info("Docker Compose service definition created successfully.")
    except NotFound:
